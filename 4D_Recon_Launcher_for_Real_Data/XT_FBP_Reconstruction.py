		...
		for i in range(int(recon['Rtime_num'][k])):
			...
			for k in range(recon['node_num']):
				...
				for j in range(k*ratio, (k+1)*ratio):
					sino_filt = radon.filter(projections[indices,:,j], ramp=1)
					Object = radon.iradon (sino_filt, angles=proj['angles'][indices], rot_ctr=proj['rotation_center_r'])/(proj['length_r']/proj['N_r'])
					print np.mean(Object)
					...
					write_array2tif(path2launch + 'object_time_z_' + str(i) + str(j) + '.tif', Object_uint8)
