...
Real_t updateVoxels_Aditya (int32_t time_begin, int32_t time_end, int32_t slice_begin, int32_t slice_end, int32_t xy_begin, int32_t xy_end, int32_t* x_rand_select, int32_t* y_rand_select, Sinogram* SinogramPtr, ScannedObject* ScannedObjectPtr, TomoInputs* TomoInputsPtr, Real_t*** ErrorSino, Real_t** DetectorResponse_XY, /*AMatrixCol* VoxelLineResponse,*/ int32_t Iter, long int *zero_count, Real_t** MagUpdateMap, uint8_t*** Mask)
{
  ...
  for (p = 0; p < maxview; p++)
  {
   ...
   for (i_new = time_begin; i_new <= time_end; i_new++) 
   {
	...
	if(ZSFlag == false)
	{
		compute_voxel_update_Aditya (SinogramPtr, ScannedObjectPtr, TomoInputsPtr, ErrorSino, AMatrixPtr, /*VoxelLineResponse,*/ Spatial_Nhood, Time_Nhood, Spatial_BDFlag, Time_BDFlag, i_new, slice, j_new, k_new);
...
/*	total_vox_mag = updateVoxels_AttTomo (time_begin, time_end, slice_begin, slice_end, xy_begin, xy_end, x_rand_select, y_rand_select, SinogramPtr, ScannedObjectPtr, TomoInputsPtr, ErrorSino, DetectorResponse_XY, VoxelLineResponse, Iter, zero_count, MagUpdateMap, Mask);*/
	total_vox_mag = updateVoxels_Aditya  (time_begin, time_end, slice_begin, slice_end, xy_begin, xy_end, x_rand_select, y_rand_select, SinogramPtr, ScannedObjectPtr, TomoInputsPtr, ErrorSino, DetectorResponse_XY, /*VoxelLineResponse,*/ Iter, zero_count, MagUpdateMap, Mask);
#endif
	return (total_vox_mag);
}
