 ...
 /* int32_t N_x, N_y, N_z, N_r, N_t;
  ...
  for (p = 0; p < maxview; p++)
  {
   ...
   for (i_new = time_begin; i_new <= time_end; i_new++) 
   {
	...
	k_new = x_rand_select[i_new][index_xy];
        j_new = y_rand_select[i_new][index_xy];
...
void upsample_bilinear_2D (ScannedObject* ScannedObjectPtr, Real_t**** Init)
{
...
void upsample_bilinear_3D (ScannedObject* ScannedObjectPtr, Real_t**** Init)
{
	int32_t i, j, k, N_time, N_x, N_y, N_z, slice;
	...
	for (i=0; i < N_time; i++){
		...
		for (j=0; j<2*N_y; j++)
			for (k=0; k<2*N_x; k++){				
				ScannedObjectPtr->Object[i][j][k][0] = buffer3D[0][j][k];
				ScannedObjectPtr->Object[i][j][k][1] = (3.0*buffer3D[0][j][k] + buffer3D[1][j][k])/4.0;
				ScannedObjectPtr->Object[i][j][k][2*N_z-1] = buffer3D[N_z-1][j][k];
				ScannedObjectPtr->Object[i][j][k][2*N_z-2] = (3.0*buffer3D[N_z-1][j][k] + buffer3D[N_z-2][j][k])/4.0;
			}
	
		for (slice=1; slice < N_z-1; slice++)	
			for (j=0; j<2*N_y; j++)
				for (k=0; k<2*N_x; k++){				
					ScannedObjectPtr->Object[i][j][k][2*slice] = (buffer3D[slice-1][j][k] + 3.0*buffer3D[slice][j][k])/4.0;
					ScannedObjectPtr->Object[i][j][k][2*slice+1] = (3.0*buffer3D[slice][j][k] + buffer3D[slice+1][j][k])/4.0;		
				}
...
void initObject(Sinogram* SinogramPtr, ScannedObject* ScannedObjectPtr, TomoInputs* TomoInputsPtr)
{
	char object_file[100];
	int dimTiff[4];
	int32_t size, result, i;
	Real_t ****Init;
	FILE *fp;	
	char initfile[] = "ObjectInit";

	for (size = 0; size < ScannedObjectPtr->N_time; size++)
		memset(&(ScannedObjectPtr->Object[size][0][0][0]), 0, ScannedObjectPtr->N_z*ScannedObjectPtr->N_y*ScannedObjectPtr->N_x*sizeof(Real_t));	
	...
	else if (TomoInputsPtr->initICD == 1 || TomoInputsPtr->initICD == 2 || TomoInputsPtr->initICD == 3)
	{
		...
		{
			size = ScannedObjectPtr->N_x*ScannedObjectPtr->N_y*ScannedObjectPtr->N_z;
			for (i = 0; i < ScannedObjectPtr->N_time; i++)
			{
				sprintf(object_file, "%s_%d.bin", OBJECT_FILENAME,i);
				fp = fopen (object_file , "r" );
				if (fp==NULL) {fprintf(TomoInputsPtr->debug_file_ptr, "ERROR: error in reading file %s\n", object_file); exit (1);}	
				result = fread (&(ScannedObjectPtr->Object[i][0][0][0]),sizeof(Real_t),size,fp);
  				if (result != size) 
				{fprintf(TomoInputsPtr->debug_file_ptr, "ERROR: Number of elements read does not match required, number of elements read=%d\n",result);}
				fclose(fp);
			}
		}
		else if (TomoInputsPtr->initICD == 3)
		{
			size = ScannedObjectPtr->N_x*ScannedObjectPtr->N_y*ScannedObjectPtr->N_z/8;
			...
			for (i = 0; i < ScannedObjectPtr->N_time; i++)
			{
				...
				result = fread (&(Init[i][0][0][0]),sizeof(Real_t),size,fp);
  				if (result != size) 
				{fprintf(TomoInputsPtr->debug_file_ptr, "ERROR: Number of elements read does not match required, number of elements read=%d\n",result);}
				fclose(fp);
			}
			upsample_bilinear_3D (ScannedObjectPtr, Init);
			for (i = 0; i < ScannedObjectPtr->N_time; i++)
				multifree(Init[i],3);
			free(Init);
		}
		...
		{
			size = ScannedObjectPtr->N_x*ScannedObjectPtr->N_y*ScannedObjectPtr->N_z/4;
			Init = (Real_t****)get_spc(ScannedObjectPtr->N_time, sizeof(Real_t***));
			for (i = 0; i < ScannedObjectPtr->N_time; i++)
			{
				sprintf(object_file, "%s_%d.bin", OBJECT_FILENAME,i);
			...
			upsample_bilinear_2D (ScannedObjectPtr, Init);
			for (i = 0; i < ScannedObjectPtr->N_time; i++)
				multifree(Init[i],3);
			free(Init);
		}
...
void initErrorSinogam (Sinogram* SinogramPtr, ScannedObject* ScannedObjectPtr, TomoInputs* TomoInputsPtr, Real_t** DetectorResponse, Real_t*** ErrorSino, AMatrixCol* VoxelLineResponse)
{
	...
	int32_t i, j, k, p, m, n, sino_idx, idx, slice;
...
void randomly_select_x_y (ScannedObject* ScannedObjectPtr, TomoInputs* TomoInputsPtr)
{
        int32_t i, j, Index, *Counter, ArraySize;
        ...
        {   
                ...
                for (j=0; j<ScannedObjectPtr->N_x*ScannedObjectPtr->N_y; j++){
                        ...
                        TomoInputsPtr->x_rand_select[i][j] = Counter[Index] % ScannedObjectPtr->N_x;
                        TomoInputsPtr->y_rand_select[i][j] = Counter[Index] / ScannedObjectPtr->N_x;
...
void update_Sinogram_Offset (Sinogram* SinogramPtr, TomoInputs* TomoInputsPtr, Real_t*** ErrorSino, Real_t** num, Real_t** den)
{
	int32_t i, j, k;
...
int updateVoxelsTimeSlices(Sinogram* SinogramPtr, ScannedObject* ScannedObjectPtr, TomoInputs* TomoInputsPtr,  Real_t** DetectorResponse, AMatrixCol* VoxelLineResponse, Real_t*** ErrorSino, int32_t Iter, uint8_t** Mask)
{
	Real_t AverageUpdate = 0, ***MagUpdateMap, **offset_numerator, **offset_denominator;
	int32_t xy_start, xy_end, i, k, num_voxel_lines, j, K;
	long int *zero_count, total_zero_count = 0;	
	int32_t* thread_num = (int32_t*)get_spc(ScannedObjectPtr->N_time, sizeof(int32_t));
    
	randomly_select_x_y (ScannedObjectPtr, TomoInputsPtr);
    
	zero_count = (long int*)get_spc(ScannedObjectPtr->N_time, sizeof(long int));
	MagUpdateMap = (Real_t***)multialloc(sizeof(Real_t), 3, ScannedObjectPtr->N_time, ScannedObjectPtr->N_y, ScannedObjectPtr->N_x);
	...
	memset(&(zero_count[0]), 0, ScannedObjectPtr->N_time*sizeof(long int));	
/*	K = ScannedObjectPtr->N_time*ScannedObjectPtr->N_z*ScannedObjectPtr->N_y*ScannedObjectPtr->N_x;
	...
	for (j = 0; j < K; j++)
	{
		num_voxel_lines = ScannedObjectPtr->N_y*ScannedObjectPtr->N_x;
		xy_start = j*floor(num_voxel_lines/K);
		xy_end = (j + 1)*floor(num_voxel_lines/K) - 1;
		xy_end = (j == K - 1) ? num_voxel_lines - 1: xy_end;
		
		#pragma omp parallel for
		for (i=0; i<ScannedObjectPtr->N_time; i=i+2)
		{					
			updateVoxels (i, i, 0, ScannedObjectPtr->N_z-1, xy_start, xy_end, TomoInputsPtr->x_rand_select, TomoInputsPtr->y_rand_select, SinogramPtr, ScannedObjectPtr, TomoInputsPtr, ErrorSino, DetectorResponse, VoxelLineResponse, Iter, &(zero_count[i]), MagUpdateMap[i], Mask);
			thread_num[i] = omp_get_thread_num();
		}
        
		#pragma omp parallel for
		for (i=1; i<ScannedObjectPtr->N_time; i=i+2)
		{			
			updateVoxels (i, i, 0, ScannedObjectPtr->N_z-1, xy_start, xy_end, TomoInputsPtr->x_rand_select, TomoInputsPtr->y_rand_select, SinogramPtr, ScannedObjectPtr, TomoInputsPtr, ErrorSino, DetectorResponse, VoxelLineResponse, Iter, &(zero_count[i]), MagUpdateMap[i], Mask);
			thread_num[i] = omp_get_thread_num();
		}
		if (TomoInputsPtr->updateProjOffset > 1)
			update_Sinogram_Offset (SinogramPtr, TomoInputsPtr, ErrorSino, offset_numerator, offset_denominator);

		num_voxel_lines = VSC_based_Voxel_Line_Select(ScannedObjectPtr, TomoInputsPtr, MagUpdateMap);			
/*		fprintf(TomoInputsPtr->debug_file_ptr, "Number of NHICD voxel lines to be updated in iteration %d is %d\n", j, num_voxel_lines);*/
		if (Iter > 1 && TomoInputsPtr->no_NHICD == 0)
		{
			...
			for (i=0; i<ScannedObjectPtr->N_time; i=i+2)
			{				
				updateVoxels (i, i, 0, ScannedObjectPtr->N_z-1, 0, num_voxel_lines-1, TomoInputsPtr->x_NHICD_select, TomoInputsPtr->y_NHICD_select, SinogramPtr, ScannedObjectPtr, TomoInputsPtr, ErrorSino, DetectorResponse, VoxelLineResponse, Iter, &(zero_count[i]), MagUpdateMap[i], Mask);
				thread_num[i] = omp_get_thread_num();
			}
        ...
        fprintf(TomoInputsPtr->debug_file_ptr, "updateVoxelsTimeSlices: Time Slice - Thread : ");
        for (i=0; i<ScannedObjectPtr->N_time; i++){
        	for (j=0; j<ScannedObjectPtr->N_y; j++){
        		for (k=0; k<ScannedObjectPtr->N_x; k++){
                		AverageUpdate += MagUpdateMap[i][j][k];
			}
		}
                total_zero_count += zero_count[i];
                fprintf(TomoInputsPtr->debug_file_ptr, "%d-%d , ", i, thread_num[i]);
        }
    
        AverageUpdate = AverageUpdate/(ScannedObjectPtr->N_z*ScannedObjectPtr->N_y*ScannedObjectPtr->N_x*ScannedObjectPtr->N_time);
        AverageUpdate = convert2Hounsfield(AverageUpdate);
            
        fprintf(TomoInputsPtr->debug_file_ptr, "\nupdateVoxelsTimeSlices: Average voxel update over all voxels is %f\n", AverageUpdate);
        fprintf(TomoInputsPtr->debug_file_ptr, "updateVoxelsTimeSlices: Zero count is %ld\n", total_zero_count);
	
	multifree(MagUpdateMap, 3);
	free(zero_count);
	free(thread_num);
...
int ICD_BackProject(Sinogram* SinogramPtr, ScannedObject* ScannedObjectPtr, TomoInputs* TomoInputsPtr)
{
	...
	{	
		flag = updateVoxelsTimeSlices (SinogramPtr, ScannedObjectPtr, TomoInputsPtr, H_r, VoxelLineResponse, ErrorSino, Iter, Mask);
#ifndef NO_COST_CALCULATE
		cost = computeCost(SinogramPtr,ScannedObjectPtr,TomoInputsPtr,ErrorSino);
